...
log = logging.getLogger("pokepricetracker.products")
    ...
    for catalog in catalog_products:
        ...
        if best_match:
            ...
            try:
                ...
                log.warning("Price fetch failed for product %s: %s", pc_id, e)
...
def get_products(token=None, set_slug="phantasmal-flames"):
    ...
    if token:
        try:
            prods = fetch_set_products(token, set_slug)
            has_live = any(p["price_source"] == "pricecharting" for p in prods)
            return prods, "pricecharting" if has_live else "no_data"
        except urllib.error.URLError as e:
            log.error("Failed to fetch products for %s: %s", set_slug, e)
